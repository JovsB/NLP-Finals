...
def detect_toxicity(sentences: list[str]):
    pipeline = joblib.load("models/toxicity_model.joblib")
    
    predictions = pipeline.predict(sentences)
    confidence_scores = pipeline.predict_proba(sentences)
    ...
    total_count = len(list(filter(lambda r: r.get('is_toxic'), results)))
